  ...
  if (!shader->IsValid()) {
    ...
    if (source->Load(it->second.file_name))
      shader->Create(std::move(source), quad_.vertex_description(),
                     quad_.primitive(), false, false, CullMode::kNone);
  }
...
void Engine::CreateRenderResources() {
  ...
  quad_ssbo_ = renderer_->CreateBuffer(pass_through_shader_.resource_id(), 0, 0,
                                       buffer_size);
  renderer_->UpdateBuffer(quad_ssbo_, vertices, buffer_size);
