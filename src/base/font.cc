...
static void StretchBlit_I8_to_RGBA32(int dst_x0, int dst_y0, int dst_x1, int dst_y1,
                                     ...
                                     uint8_t *dst_rgba, int dst_pitch,
                                     const uint8_t *src_i, int src_pitch) {
  // LOG("-- StretchBlit: --\n"
  ...
  while (*text) {
    if (*text >= kFirstChar /*&& *text < (kFirstChar + kNumChars)*/) {
      ...
      int ix0 = (int)q.x0,
          ...
          ix1 = (int)q.x1,
          iy1 = (int)q.y1,
          iu0 = (int)(q.s0 * kGlyphSize),
          iv0 = (int)(q.t0 * kGlyphSize),
          iu1 = (int)(q.s1 * kGlyphSize),
          iv1 = (int)(q.t1 * kGlyphSize);
