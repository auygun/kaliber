...
class Vector3 {
 public:
  ...
  void Combine(const Vector3& a, T t, Vector3& dest) const {
  ...
  // Reflect this on the plane defined by n
...
class Plane {
 public:
  ...
  void CreateND(const Vector3<T>& _n, T _d) {
    n = _n;
    d = _d;
  }

  void CreateNV(const Vector3<T>& _n, const Vector3<T>& _p) {
    n = _n;
    d = -(_n * _p);
  }

  void Create(const Vector3<T>& _p0,
              const Vector3<T>& _p1,
              const Vector3<T>& _p2) {
    Vector3<T> v0 = _p1 - _p0;
    Vector3<T> v1 = _p2 - _p0;
    n = (v0 / v1).Normalize();
    d = -(n * _p0);
  }

  void CreateInverse(const Plane& _Plane) {
    n = -_Plane.n;
    d = -_Plane.d;
  }
  ...
  Vector3<T> GetPointInPlane() const { return n * (-d); }

  T Distance(const Vector3<T>& v) const {
    return (v.k[0] * n.k[0] + v.k[1] * n.k[1] + v.k[2] * n.k[2] + d);
  }

  bool AlmostEqual(const Plane& _p1,
                   T _EpsilonD = 0.0001,
                   T _EpsilonN = 0.0001) const {
  ...
  int GetPlaneSide(const Vector3<T>& _v) const {
  ...
  int GetPlaneSide_Epsilon(const Vector3<T>& _v, T _Epsilon) const {
    // retur: 1 = normal sidan.
    //		 -1 = baksidan
    //        0 = i planet.
  ...
  int GetPlaneSideMask_Epsilon(const Vector3<T>& _v, T _Epsilon) const {
    // retur: 1 = normal sidan.
    //		  2 = baksidan
    //        4 = i planet.
    T dist = Distance(_v);
    if (dist < (T)-_Epsilon)
      return 2;
    else if (dist > (T)_Epsilon)
      return 1;
    return 4;
  }

  int GetArrayPlaneSideMask(const Vector3<T>* _pV, int _nv) const {
    // retur: 1 = normal sidan.
    //		  2 = baksidan
  ...
  int GetArrayPlaneSideMask_Epsilon(const Vector3<T>* _pV,
                                    int _nv,
                                    T _Epsilon) const {
    // retur: 1 = normal sidan.
    //		  2 = baksidan
  ...
  int GetBoxPlaneSideMask(const Vector3<T>& _VMin,
                          const Vector3<T>& _VMax) const {
    // retur: 1 = normal sidan.
    //		  2 = baksidan
    ...
    xMin = Sel(n.k[0], _VMin.k[0], _VMax.k[0]) * n.k[0];
    xMax = Sel(n.k[0], _VMax.k[0], _VMin.k[0]) * n.k[0];

    //		if (n.k[1] > T(0.0))
    //			{ yMin = _VMin.k[1]*n.k[1]; yMax = _VMax.k[1]*n.k[1]; }
    //		else
    //			{ yMin = _VMax.k[1]*n.k[1]; yMax = _VMin.k[1]*n.k[1]; }
    yMin = Sel(n.k[1], _VMin.k[1], _VMax.k[1]) * n.k[1];
    yMax = Sel(n.k[1], _VMax.k[1], _VMin.k[1]) * n.k[1];

    //		if (n.k[2] > T(0.0))
    //			{ zMin = _VMin.k[2]*n.k[2]; zMax = _VMax.k[2]*n.k[2]; }
    //		else
    //			{ zMin = _VMax.k[2]*n.k[2]; zMax = _VMin.k[2]*n.k[2]; }
    zMin = Sel(n.k[2], _VMin.k[2], _VMax.k[2]) * n.k[2];
    zMax = Sel(n.k[2], _VMax.k[2], _VMin.k[2]) * n.k[2];

    if (xMin + yMin + zMin + d > T(0.0))
      return 1;
    if (xMax + yMax + zMax + d > T(0.0))
      return 3;
    return 2;
  }

  T GetBoxMinDistance(const Vector3<T>& _VMin, const Vector3<T>& _VMax) const {
    /*		fp32 Result = d;
    if (FloatIsNeg(n[0]))
    ...
    */
    ...
    T xMin, yMin, zMin;
    //		xMin = (n.k[0] > T(0.0)) ? _VMin.k[0]*n.k[0] :
    //_VMax.k[0]*n.k[0]; 		yMin = (n.k[1] > T(0.0)) ?
    //_VMin.k[1]*n.k[1] : _VMax.k[1]*n.k[1]; 		zMin = (n.k[2] > T(0.0))
    //? _VMin.k[2]*n.k[2] : _VMax.k[2]*n.k[2];
    xMin = Sel(n.k[0], _VMin.k[0], _VMax.k[0]) * n.k[0];
    yMin = Sel(n.k[1], _VMin.k[1], _VMax.k[1]) * n.k[1];
    zMin = Sel(n.k[2], _VMin.k[2], _VMax.k[2]) * n.k[2];
    return xMin + yMin + zMin + d;
  ...
  T GetBoxMaxDistance(const Vector3<T>& _VMin, const Vector3<T>& _VMax) const {
    /*		fp32 Result = d;
    if (FloatIsNeg(n[0]))
    Result += _VMin.k[0]*n.k[0];
    else
    ...
    T xMax, yMax, zMax;
    //		xMax = (n.k[0] < T(0.0)) ? _VMin.k[0]*n.k[0] :
    //_VMax.k[0]*n.k[0]; 		yMax = (n.k[1] < T(0.0)) ?
    //_VMin.k[1]*n.k[1] : _VMax.k[1]*n.k[1]; 		zMax = (n.k[2] < T(0.0))
    //? _VMin.k[2]*n.k[2] : _VMax.k[2]*n.k[2];
    xMax = Sel(n.k[0], _VMax.k[0], _VMin.k[0]) * n.k[0];
    yMax = Sel(n.k[1], _VMax.k[1], _VMin.k[1]) * n.k[1];
    zMax = Sel(n.k[2], _VMax.k[2], _VMin.k[2]) * n.k[2];
  ...
  void GetBoxMinAndMaxDistance(const Vector3<T>& _VMin,
                               const Vector3<T>& _VMax,
                               T& _MinDistance,
                               T& _MaxDistance) const {
    /*		fp32 MinDist = d;
    fp32 MaxDist = d;
  ...
  bool IntersectsPlane(const Vector3<T>& _p0,
                       const Vector3<T>& _p1,
                       Vector3<T>& _RetV) const {
  ...
  bool IntersectLineSegment(const Vector3<T>& _p0,
                            const Vector3<T>& _p1,
                            Vector3<T>& _RetV) const {
    T dvx = (_p1.k[0] - _p0.k[0]);
    T dvy = (_p1.k[1] - _p0.k[1]);
  ...
  void GetIntersectionPoint(const Vector3<T>& _p0,
                            const Vector3<T>& _p1,
                            Vector3<T>& _RetV) const {
    T dvx = (_p1.k[0] - _p0.k[0]);
    T dvy = (_p1.k[1] - _p0.k[1]);
  ...
  void Translate(const Vector3<T>& _dV) { d -= (n * _dV); }

  void Transform(const Matrix4<T>& _Mat) {
    n.MultiplyMatrix3x3(_Mat);
    Translate(Vector3<T>::GetMatrixRow(_Mat, 3));
  }

  //---------------------------------------------------
  void ReflectMatrix3x3(const Matrix4<T>& _Mat, Matrix4<T>& _RefMat) const {
    // Reflects _Mat in _Plane and puts the result in _RefMat
    _RefMat.UnitNot3x3();
    _Mat.Row(0).Reflect(n, _RefMat.Row(0));
    _Mat.Row(1).Reflect(n, _RefMat.Row(1));
    _Mat.Row(2).Reflect(n, _RefMat.Row(2));
  }

  void ReflectMatrix(const Matrix4<T>& _Mat, Matrix4<T>& _RefMat) const {
    // Reflects _Mat in _Plane and puts the result in _RefMat
    _RefMat.UnitNot3x3();
    _Mat.Row(0).Reflect(n, _RefMat.Row(0));
    _Mat.Row(1).Reflect(n, _RefMat.Row(1));
    _Mat.Row(2).Reflect(n, _RefMat.Row(2));
    _Mat.Row(3).Combine(n, -T(2.0) * Distance(_Mat.Row(3)), _RefMat.Row(3));
  }

  std::string ToString() {
    using namespace std::string_literals;
    return "(n:"s + std::to_string(n.k[0]) + ", "s + std::to_string(n.k[1]) +
           std::to_string(n.k[2]) + ", "s + std::to_string(n.k[3]) + ") d:"s +
           std::to_string(d);
  }
};
...
class AABB {
 public:
  ...
  AABB(const Vector3<T>& _Min, const Vector3<T>& _Max) {
    m_Min = _Min;
    m_Max = _Max;
  }

  void And(const AABB& _Box) {
    m_Min.k[0] = std::max(m_Min.k[0], _Box.m_Min.k[0]);
    m_Min.k[1] = std::max(m_Min.k[1], _Box.m_Min.k[1]);
    m_Min.k[2] = std::max(m_Min.k[2], _Box.m_Min.k[2]);

    m_Max.k[0] = std::min(m_Max.k[0], _Box.m_Max.k[0]);
    m_Max.k[1] = std::min(m_Max.k[1], _Box.m_Max.k[1]);
    m_Max.k[2] = std::min(m_Max.k[2], _Box.m_Max.k[2]);
  }

  void And(const AABB& _Box, AABB& _Dst) const {
    _Dst.m_Min.k[0] = std::max(m_Min.k[0], _Box.m_Min.k[0]);
    _Dst.m_Min.k[1] = std::max(m_Min.k[1], _Box.m_Min.k[1]);
    _Dst.m_Min.k[2] = std::max(m_Min.k[2], _Box.m_Min.k[2]);

    _Dst.m_Max.k[0] = std::min(m_Max.k[0], _Box.m_Max.k[0]);
    _Dst.m_Max.k[1] = std::min(m_Max.k[1], _Box.m_Max.k[1]);
    _Dst.m_Max.k[2] = std::min(m_Max.k[2], _Box.m_Max.k[2]);
  }

  void Expand(const AABB& _Box) {
    m_Min.k[0] = std::min(m_Min.k[0], _Box.m_Min.k[0]);
    m_Min.k[1] = std::min(m_Min.k[1], _Box.m_Min.k[1]);
    m_Min.k[2] = std::min(m_Min.k[2], _Box.m_Min.k[2]);

    m_Max.k[0] = std::max(m_Max.k[0], _Box.m_Max.k[0]);
    m_Max.k[1] = std::max(m_Max.k[1], _Box.m_Max.k[1]);
    m_Max.k[2] = std::max(m_Max.k[2], _Box.m_Max.k[2]);
  }

  void Expand(const Vector3<T>& _v) {
    m_Min.k[0] = std::min(m_Min.k[0], _v.k[0]);
    m_Max.k[0] = std::max(m_Max.k[0], _v.k[0]);

    m_Min.k[1] = std::min(m_Min.k[1], _v.k[1]);
    m_Max.k[1] = std::max(m_Max.k[1], _v.k[1]);

    m_Min.k[2] = std::min(m_Min.k[2], _v.k[2]);
    m_Max.k[2] = std::max(m_Max.k[2], _v.k[2]);
  }

  bool operator==(const AABB<T>& _rhs) const {
    return (m_Min == _rhs.m_Min) && (m_Max == _rhs.m_Max);
  }

  bool operator!=(const AABB<T>& _rhs) const { return !((*this) == _rhs); }
  ...
  void Transform(const Matrix4<T>& _Mat, AABB& _Dest) const {
    // Transforms and expands box so that it still is a valid bounding box for
    // the original space.
    Vector3<T> E;
    Vector3<T> C;
    ...
    for (int k = 0; k < 3; k++)
      _Dest.m_Max.k[k] = Abs(_Mat.k[0][k] * E[0]) + Abs(_Mat.k[1][k] * E[1]) +
                         Abs(_Mat.k[2][k] * E[2]);
  ...
  bool IsInside(const AABB& _Box) const {
    // True if _Box intersects this box.
    if ((_Box.m_Max.k[0] < m_Min.k[0]) || (_Box.m_Max.k[1] < m_Min.k[1]) ||
        (_Box.m_Max.k[2] < m_Min.k[2]))
      return false;

    if ((_Box.m_Min.k[0] > m_Max.k[0]) || (_Box.m_Min.k[1] > m_Max.k[1]) ||
        (_Box.m_Min.k[2] > m_Max.k[2]))
      return false;

    return true;
  }

  bool IsNearlyInside(const AABB& _Box, const float tolerance) const {
    // True if _Box intersects this box.
    if ((_Box.m_Max.k[0] - tolerance < m_Min.k[0]) ||
        (_Box.m_Max.k[1] - tolerance < m_Min.k[1]) ||
        (_Box.m_Max.k[2] - tolerance < m_Min.k[2]))
      return false;

    if ((_Box.m_Min.k[0] + tolerance > m_Max.k[0]) ||
        (_Box.m_Min.k[1] + tolerance > m_Max.k[1]) ||
        (_Box.m_Min.k[2] + tolerance > m_Max.k[2]))
      return false;

    return true;
  }

  bool IsPointInside(const Vector3<T>& _Point) const {
    return _Point[0] >= m_Min[0] && _Point[0] <= m_Max[0] &&
           _Point[1] >= m_Min[1] && _Point[1] <= m_Max[1] &&
           _Point[2] >= m_Min[2] && _Point[2] <= m_Max[2];
  }
  bool IsCovering(const AABB& _Box) const {
    // True if this box fit inside _Box
    if ((_Box.m_Max.k[0] < m_Max.k[0]) || (_Box.m_Max.k[1] < m_Max.k[1]) ||
        (_Box.m_Max.k[2] < m_Max.k[2]))
      return false;

    if ((_Box.m_Min.k[0] > m_Min.k[0]) || (_Box.m_Min.k[1] > m_Min.k[1]) ||
        (_Box.m_Min.k[2] > m_Min.k[2]))
      return false;

    return true;
  }

  void GetCenter(Vector3<T>& _Center) const {
    _Center.k[0] = (m_Max.k[0] + m_Min.k[0]) / 2;
    _Center.k[1] = (m_Max.k[1] + m_Min.k[1]) / 2;
    _Center.k[2] = (m_Max.k[2] + m_Min.k[2]) / 2;
  }

  bool InVolume(const Plane<T>* _pP, int _nP) const {
    Vector3<T> C;
    Vector3<T> E;
    m_Max.Lerp(m_Min, (T)0.5, C);
    m_Max.Sub(C, E);
    ...
    for (int ip = 0; ip < _nP; ip++) {
      const Plane<T>* pP = &_pP[ip];
      T d =
          C * pP->n + pP->d -
          (Abs(E[0] * pP->n[0]) + Abs(E[1] * pP->n[1]) + Abs(E[2] * pP->n[2]));
      if (d > (T)0.0)
        return false;
    }
    return true;
  ...
  T GetMinSqrDistance(const Vector3<T>& _v) const {
    Vector3<T> dV;
    for (int i = 0; i < 3; i++) {
      if (_v.k[i] > m_Min.k[i]) {
        if (_v.k[i] > m_Max.k[i])
  ...
  T GetMaxSqrDistance(const Vector3<T>& _v) const {
    Vector3<T> dV;
    for (int i = 0; i < 3; i++) {
  ...
  T GetMinDistance(const Vector3<T>& _v) const {
    return std::sqrt(GetMinSqrDistance(_v));
  }

  T GetMaxDistance(const Vector3<T>& _v) const {
    return std::sqrt(GetMaxSqrDistance(_v));
  }

  bool IntersectLine(const Vector3<T>& _p0,
                     const Vector3<T>& _p1,
                     Vector3<T>& _RetHitPos) const {
    Vector3<T> p0(_p0);
    Vector3<T> p1(_p1);
  ...
  T GetMinProjection(const Vector3<T>& _Vec) const {
    Vector3<T> HDim = (m_Max - m_Min) / T(2);
    Vector3<T> Cen = m_Min + HDim;
    T HDimProj = Abs(_Vec.k[0]) * HDim.k[0] + Abs(_Vec.k[1]) * HDim.k[1] +
                 Abs(_Vec.k[2]) * HDim.k[2];
    return (_Vec * Cen) - HDimProj;
  }

  T GetMaxProjection(const Vector3<T>& _Vec) const {
    Vector3<T> HDim = (m_Max - m_Min) / T(2);
    Vector3<T> Cen = m_Min + HDim;
    T HDimProj = Abs(_Vec.k[0]) * HDim.k[0] + Abs(_Vec.k[1]) * HDim.k[1] +
                 Abs(_Vec.k[2]) * HDim.k[2];
  ...
  T GetMinMaxProjection(const Vector3<T>& _Vec,
                        T& _MinProj,
                        T& _MaxProj) const {
    Vector3<T> HDim = (m_Max - m_Min) / T(2);
    Vector3<T> Cen = m_Min + HDim;
    ...
    T CenProj = _Vec * Cen;
    _MinProj = CenProj - HDimProj;
    _MaxProj = CenProj + HDimProj;
    return CenProj;
  }

  std::string ToString() {
    using namespace std::string_literals;
    return "("s + std::to_string(m_Min.k[0]) + ", "s +
           std::to_string(m_Min.k[1]) + ", "s + std::to_string(m_Min.k[2]) +
           "), ("s + std::to_string(m_Max.k[0]) + ", "s +
           std::to_string(m_Max.k[1]) + ", "s + std::to_string(m_Max.k[2]) +
           ")"s;
  }
};
...
class OBB {
 public:
  Vector3<T> m_A[3];  // Axis
  Vector3<T> m_E;     // Extents
  Vector3<T> m_C;     // Center

  void Create(const Vector3<T>& _E, const Vector3<T>& _C) {
    m_E = _E;
    m_C = _C;
    m_A[0] = Vector3<T>(1, 0, 0);
    m_A[1] = Vector3<T>(0, 1, 0);
    m_A[2] = Vector3<T>(0, 0, 1);
  }

  void Create(const Vector3<T>& _E,
              const Vector3<T>& _C,
              const Matrix4<T>& _Mat) {
    m_E = _E;
    m_C = _C;
    m_A[0] = Vector3<T>::GetMatrixRow(_Mat, 0);
    m_A[1] = Vector3<T>::GetMatrixRow(_Mat, 1);
    m_A[2] = Vector3<T>::GetMatrixRow(_Mat, 2);
  }

  void Create(const AABB<T>& _Box) {
  ...
  OBB(const AABB<T>& _Box) { Create(_Box); }

  void SetDimensions(const Vector3<T>& _E) { m_E = _E; }

  void SetPosition(const Matrix4<T>& _Pos) {
    m_C = Vector3<T>::GetRow(_Pos, 3);
    m_A[0] = Vector3<T>::GetRow(_Pos, 0);
    m_A[1] = Vector3<T>::GetRow(_Pos, 1);
    m_A[2] = Vector3<T>::GetRow(_Pos, 2);
  }

  void operator+=(const Vector3<T>& _v) { m_C += _v; }
  ...
  void Transform(const Matrix4<T>& _Mat, OBB& _Dest) const {
    _Dest.m_A[0] = m_A[0];
  ...
  void GetVertices(Vector3<T>* _pV) const {
    for (int k = 0; k < 3; k++) {
      _pV[0][k] = m_C[k] - (m_E[0] * m_A[0][k]) - (m_E[1] * m_A[1][k]) -
                  (m_E[2] * m_A[2][k]);
      _pV[1][k] = m_C[k] + (m_E[0] * m_A[0][k]) - (m_E[1] * m_A[1][k]) -
                  (m_E[2] * m_A[2][k]);
      _pV[2][k] = m_C[k] + (m_E[0] * m_A[0][k]) + (m_E[1] * m_A[1][k]) -
                  (m_E[2] * m_A[2][k]);
      _pV[3][k] = m_C[k] - (m_E[0] * m_A[0][k]) + (m_E[1] * m_A[1][k]) -
                  (m_E[2] * m_A[2][k]);
      _pV[4][k] = m_C[k] - (m_E[0] * m_A[0][k]) - (m_E[1] * m_A[1][k]) +
                  (m_E[2] * m_A[2][k]);
      _pV[5][k] = m_C[k] + (m_E[0] * m_A[0][k]) - (m_E[1] * m_A[1][k]) +
                  (m_E[2] * m_A[2][k]);
      _pV[6][k] = m_C[k] + (m_E[0] * m_A[0][k]) + (m_E[1] * m_A[1][k]) +
                  (m_E[2] * m_A[2][k]);
      _pV[7][k] = m_C[k] - (m_E[0] * m_A[0][k]) + (m_E[1] * m_A[1][k]) +
                  (m_E[2] * m_A[2][k]);
    }
  }

  void GetBoundBox(AABB<T>& _Box) const {
    for (int k = 0; k < 3; k++)
      _Box.m_Max.k[k] = Abs(m_A[0][k] * m_E[0]) + Abs(m_A[1][k] * m_E[1]) +
                        Abs(m_A[2][k] * m_E[2]);
  ...
  void GetLocalBox(AABB<T>& _Box) const {
    _Box.m_Min = -m_E;
    _Box.m_Max = m_E;
  }

  void TransformToBoxSpace(const Vector3<T>& _v, Vector3<T>& _Dst) const {
    T vx = _v[0] - m_C[0];
    T vy = _v[1] - m_C[1];
    T vz = _v[2] - m_C[2];
  ...
  void TransformFromBoxSpace(const Vector3<T>& _v, Vector3<T>& _Dst) const {
  ...
  void TransformToBoxSpace_Vector(const Vector3<T>& _v,
                                  Vector3<T>& _Dst) const {
    T vx = _v[0];
    T vy = _v[1];
    T vz = _v[2];
    T dstx = vx * m_A[0][0] + vy * m_A[0][1] + vz * m_A[0][2];
  ...
  void TransformFromBoxSpace_Vector(const Vector3<T>& _v,
                                    Vector3<T>& _Dst) const {
    T dstx = _v.k[0] * m_A[0][0] + _v.k[1] * m_A[1][0] + _v.k[2] * m_A[2][0];
    T dsty = _v.k[0] * m_A[0][1] + _v.k[1] * m_A[1][1] + _v.k[2] * m_A[2][1];
    T dstz = _v.k[0] * m_A[0][2] + _v.k[1] * m_A[1][2] + _v.k[2] * m_A[2][2];
  ...
  bool LocalPointInBox(const Vector3<T>& _p0) const {
    for (int i = 0; i < 3; i++) {
      if (_p0.k[i] > m_E[i])
        return false;
      if (_p0.k[i] < -m_E[i])
  ...
  bool LocalIntersectLine(const Vector3<T>& _p0,
                          const Vector3<T>& _p1,
                          Vector3<T>& _HitPos) const {
  ...
  T LocalMinSqrDistance(const Vector3<T>& _v) const {
    // _v is in box-space
    Vector3<T> dV;
    for (int i = 0; i < 3; i++) {
      if (_v.k[i] > -m_E.k[i]) {
  ...
  T LocalMaxSqrDistance(const Vector3<T>& _v) const {
    // _v is in box-space
    Vector3<T> dV;
    for (int i = 0; i < 3; i++) {
      if (_v.k[i] > 0)
  ...
  // ---------------------------------------------------------------------------
  T GetMinDistance(const Plane<T>& _Plane) const {
    T d = _Plane.Distance(m_C);
    for (int k = 0; k < 3; k++)
      d -= Abs(_Plane.n * m_A[k]) * m_E[k];
    return d;
  }

  T GetMaxDistance(const Plane<T>& _Plane) const {
    T d = _Plane.Distance(m_C);
    for (int k = 0; k < 3; k++)
      d += Abs(_Plane.n * m_A[k]) * m_E[k];
    return d;
  ...
  //   return CStrF(
  //       "C:(%.3f,%.3f,%.3f) E:(%.3f,%.3f,%.3f) A0:(%.3f,%.3f,%.3f) "
  //       "A1:(%.3f,%.3f,%.3f) A2:(%.3f,%.3f,%.3f)",
  //       m_C[0], m_C[1], m_C[2], m_E[0], m_E[1], m_E[2], m_A[0][0], m_A[0][1],
  //       m_A[0][2], m_A[1][0], m_A[1][1], m_A[1][2], m_A[2][0], m_A[2][1],
  //       m_A[2][2]);
  // }
};
